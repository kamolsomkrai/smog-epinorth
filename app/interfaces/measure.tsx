...
export interface Measure3Data {
  province: string;
  measure3_pollution_clinic_open: number; // 3.1 เปิดคลินิกมลพิษ
  measure3_pollution_clinic_service: number; // 3.2 ผู้รับบริการคลินิกมลพิษ
  measure3_online_clinic_open: number; // 3.3 เปิดบริการคลินิกมลพิษออนไลน์
  measure3_online_clinic_service: number; // 3.4 ผู้รับบริการคลินิกมลพิษออนไลน์
  measure3_nursery_dust_free_open: number; // 3.5 เปิดบริการห้องปลอดฝุ่น Nursery
  measure3_nursery_dust_free_service: number; // 3.6 ผู้รับบริการห้องปลอดฝุ่น Nursery
  measure3_gov_dust_free_open: number; // 3.7 เปิดบริการห้องปลอดฝุ่น รัฐ
  measure3_gov_dust_free_service: number; // 3.8 ผู้รับบริการห้องปลอดฝุ่น รัฐ
  measure3_active_teams_3_doctors_total: number; // 3.9 ทีม 3 หมอ (รวม)
  measure3_active_teams_3_doctors_add: number; // 3.10 ทีม 3 หมอ (เพิ่ม)
  measure3_active_teams_mobile_total: number; // 3.11 ทีมหน่วยแพทย์เคลื่อนที่ (รวม)
  measure3_active_teams_mobile_add: number; // 3.12 ทีมหน่วยแพทย์เคลื่อนที่ (เพิ่ม)
  measure3_active_teams_citizens_total: number; // 3.13 ทีมหน่วยดูแลประชาชน (รวม)
  measure3_active_teams_citizens_add: number; // 3.14 ทีมหน่วยดูแลประชาชน (เพิ่ม)
  measure3_personal_protective_gear: number; // 3.15 อุปกรณ์ป้องกันส่วนบุคคล
  measure3_elderly_N95_mask: number; // 3.16 หน้ากาก N95 สำหรับผู้สูงอายุ
  measure3_elderly_surgical_mask: number; // 3.17 หน้ากากอนามัยสำหรับผู้สูงอายุ
  measure3_children_N95_mask: number; // 3.18 หน้ากาก N95 สำหรับเด็กเล็ก
  measure3_children_surgical_mask: number; // 3.19 หน้ากากอนามัยสำหรับเด็กเล็ก
  measure3_pregnant_N95_mask: number; // 3.20 หน้ากาก N95 สำหรับหญิงตั้งครรภ์
  measure3_pregnant_surgical_mask: number; // 3.21 หน้ากากอนามัยสำหรับหญิงตั้งครรภ์
  measure3_bedridden_N95_mask: number; // 3.22 หน้ากาก N95 สำหรับติดเตียง
  measure3_bedridden_surgical_mask: number; // 3.23 หน้ากากอนามัยสำหรับติดเตียง
  measure3_disease_N95_mask: number; // 3.24 หน้ากาก N95 สำหรับผู้มีโรคประจำตัว
  measure3_disease_surgical_mask: number; // 3.25 หน้ากากอนามัยสำหรับผู้มีโรคประจำตัว
  measure3_sky_doctor: number; // 3.26 SKY doctor
  measure3_ambulance: number; // 3.27 รถกู้ชีพ ALS
  measure3_health_check_staff: number;
  measure3_health_check_volunteer: number;
}

// interfaces/measure.ts
export interface Measure4Data {
  province: string;
  measure4_eoc_open_date: string; // รูปแบบ YYYY-MM-DD
  measure4_eoc_close_date: string; // รูปแบบ YYYY-MM-DD
  measure4_law_enforcement_fine: number; // จับปรับ (บาท)
}
